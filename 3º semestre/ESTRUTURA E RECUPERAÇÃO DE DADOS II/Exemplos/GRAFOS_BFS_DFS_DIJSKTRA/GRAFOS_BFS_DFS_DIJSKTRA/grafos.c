...
void bfsCaminho(const Grafo *grafo, const TipoItem inicio, const TipoItem fim){
    ...
    bool *descoberto = malloc(grafo->numvertices * sizeof(bool));
    ...
    descoberto[indiceInicio] = true; //Toda vez que um vértice é enfileirado, ele é marcado como descoberto. Não pode ser enfileirado novamente.
    ...
    while (!filaVazia(fila)) {
        ...
        if (verticeAtual == indiceFim) {
            ...
            encontrou = true;
            printf("O caminho encontrado é: FIM <- ");

            int atual = indiceFim;
            while(atual!= -1)
            {
                printf("%c <- ",grafo->vertices[atual]);
                atual = pai[atual];
            }
            printf("INICIO");
        ...
        for (int i = 0; i < grafo->numvertices; i++) {
            if ((grafo->matrizadjacencias[verticeAtual][i] != grafo->arestanula) && !descoberto[i]) {
...
bool *descoberto = malloc(grafo->numvertices * sizeof(bool));
int *pai = malloc(grafo->numvertices * sizeof(int));
...
descoberto[indiceInicio] = true; //Toda vez que um vértice é empilhado, ele é marcado como descoberto. Não pode ser empilhado novamente.
bool encontrou = false;
...
while (!pilhaVazia(pilha)) {
    ...
    printf("Visitando vertice (Desempilhando): %c\n", grafo->vertices[verticeAtual]); //O vértice entra na ordem de visitação no momento em que é desempilhado.
...
int menorDistancia(int* distancias, bool* explorado, int n) { //Retorna o índice do vértice com a menor distância que ainda não foi explorado
    int menor = INT_MAX; 
    int indiceMenor = -1;

    for (int i = 0; i < n; i++) {
        if (!explorado[i] && distancias[i] < menor) {
            menor = distancias[i];
            indiceMenor = i;
        }
    }
...
void dijkstra(const Grafo *grafo, TipoItem inicio, int* distancias, int* pais) {

    int indiceInicio = obterindice(grafo, inicio);
    int n = grafo->numvertices;

    bool *explorado = malloc(n * sizeof(bool));
    ...
    pais[indiceInicio] = indiceInicio;

    for (int cont = 0; cont < n; cont++) { // Cada iteração é a exploração de um vértice.
        int atual = menorDistancia(distancias, explorado, n);

        if (atual == -1) { // Todos os vértices restantes têm distância infinita ou já foram explorados
            break;
        }

        explorado[atual] = true;

        for (int i = 0; i < n; i++) {
            int peso = grafo->matrizadjacencias[atual][i];

            if (peso != grafo->arestanula && !explorado[i]) {
                if (distancias[atual] != INT_MAX &&
                    distancias[atual] + peso < distancias[i]) { // Verifica se o caminho passando por 'atual' é mais curto do que o caminho conhecido para 'i'

                    distancias[i] = distancias[atual] + peso; // Atualiza a distância para 'i' se o caminho passando por 'atual' for mais curto
                    pais[i] = atual; // Atualiza o pai de 'i' para 'atual', indicando que o caminho mais curto para 'i' passa por 'atual'
                }
            }
        }
    }
...
void imprimirCaminhoMinimo(const Grafo *grafo, 
    TipoItem inicio, 
    TipoItem fim, 
    int *distancias, 
    int *pais) {
...
if (distancias[indiceFim] == INT_MAX) {
printf("Nao existe caminho entre %c e %c!\n", inicio, fim);
return;
}

printf("Caminho minimo encontrado: FIM <- ");
